...
import static com.hyphenate.chatuidemo.ui.ContextMenuActivity.RESULT_CEODE_BABA;
...
public class ChatFragment extends EaseChatFragment implements EaseChatFragmentHelper {
        ...
        if (requestCode == REQUEST_CODE_CONTEXT_MENU) {
            switch (resultCode) {
                ...
                case ContextMenuActivity.RESULT_CODE_FORWARD: // forward
                    ...
                    startActivity(intent);
                    break;
                case ContextMenuActivity.RESULT_CODE_CHEHUI:
                    // 1.create a cmd message
                    EMMessage cmdMessage = EMMessage.createSendMessage(EMMessage.Type.CMD);
                    EMCmdMessageBody body = new EMCmdMessageBody("backMsg");
                    cmdMessage.addBody(body);

                    // 2.set attr
                    String desConversation = "";
                    if (chatType == EaseConstant.CHATTYPE_SINGLE){
                        desConversation = PreferenceManager.getInstance().getCurrentUsername();
                    } else if (chatType == EaseConstant.CHATTYPE_GROUP){
                        desConversation = toChatUsername;
                    }
                    cmdMessage.setAttribute("msgId", longClickMessage.getMsgId());
                    cmdMessage.setAttribute("desConversation", desConversation);
                    ...
                    conversation.removeMessage(longClickMessage.getMsgId());
                    break;

                case RESULT_CEODE_BABA:
                    /*EMMessage emMessage = EMMessage.createTxtSendMessage("我是一只大黄狗", toChatUsername);
                    EMClient.getInstance().chatManager().sendMessage(emMessage);*/
                    Toast.makeText(getActivity(), "我是一只大黄狗", Toast.LENGTH_SHORT).show();
                default:
                    break;
            }
        }
    ...
    EMMessage longClickMessage;

    /**
     * 长按对应的消息，然后就是弹出对应的activity，然后去选择
     * @param message
     */
    @Override
    public void onMessageBubbleLongClick(EMMessage message) {
        // no message forward when in chat room
        longClickMessage = message;
